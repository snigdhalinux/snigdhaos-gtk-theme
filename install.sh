...
if [ "$UID" -eq "$ROOT_UID" ]; then
  DESTINATION_DIRECTORY="/usr/share/themes"
else
...
OPTIONS:
  -d, --dest DIR          Specify destination directory (Default: $DESTINATION_DIRECTORY)
...
install() {
  ...
  echo "Comment=Snigdha OS Gtk+ Theme" >>                                       "$THEME_DIR/index.theme"
  ...
  if [[ "$tweaks" == 'true' ]]; then
    sassc $SASSC_OPT "$SRC_DIR/gnome-shell/shell-$GSHELL_VERSION/gnome-shell$color$size.scss" "$THEME_DIR/gnome-shell/gnome-shell.css"
  else
    cp -r "$SRC_DIR/gnome-shell/shell-$GSHELL_VERSION/gnome-shell$color$size.css"   "$THEME_DIR/gnome-shell/gnome-shell.css"
  fi
    ...
    for theme in "${THEME_VARIANTS[@]}"; do
      for color in '-light' '-dark'; do
        for size in "${SIZE_VARIANTS[@]}"; do
          clean "${dest:-$DESTINATION_DIRECTORY}" "${_name:-$THEME_NAME}" "$round" "$theme" "$color" "$size"
        done
      done
    done
  done
  ...
  for theme in "${themes[@]}"; do
    for color in "${colors[@]}"; do
      for size in "${sizes[@]}"; do
        uninstall "${dest:-$DESTINATION_DIRECTORY}" "${_name:-$THEME_NAME}" "$theme" "$color" "$size"
      done
    done
  done
}
...
install_theme() {
  for theme in "${themes[@]}"; do
    for color in "${colors[@]}"; do
      for size in "${sizes[@]}"; do
        install "${dest:-$DESTINATION_DIRECTORY}" "${_name:-$THEME_NAME}" "$theme" "$color" "$size" "$icon"
      done
    done
  done
}
  ...
  for theme in "${themes[@]}"; do
    for color in "${lcolors[@]}"; do
      for size in "${sizes[0]}"; do
        link_libadwaita "${dest:-$DESTINATION_DIRECTORY}" "${_name:-$THEME_NAME}" "$theme" "$color" "$size"
      done
    done
  done
}
